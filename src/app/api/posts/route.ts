    ...
    if (tagNames && Array.isArray(tagNames) && tagNames.length > 0) {
      for (const tagName of tagNames.slice(0, 10)) {
        ...
        if (!tag) {
          ...
          tag = { id: tagId, name: trimmedName, slug, postsCount: 1 } as typeof tags.$inferSelect;
        } else {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = data.map((post) => ({
      ...
      files: postFiles.filter((f) => f.postId === post.id).map((f) => ({ id: f.id, fileName: f.fileName, originalName: f.originalName, fileUrl: f.fileUrl, fileSize: f.fileSize, mimeType: f.mimeType, fileType: f.fileType, thumbnailUrl: f.thumbnailUrl })),
      tags: postTagsData.filter((t) => t.postId === post.id).map((t) => ({ id: t.id, name: t.name, slug: t.slug })),
